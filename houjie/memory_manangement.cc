...
// 這無所謂，因為每個 standard headers 都有自我防衛機制，不讓自己被 included
// 二次.
//
// 本文件用到若干 GNU C++ extended allocators，所以你或許需要在你的集成環境
// (IDE) 上設定 "C++11 on". 我已將相關代碼包裝在 編譯條件選項 __GNUC__ 中。
...
void test_call_ctor_directly() {
  ...
  //! pstr->~string();	//crash -- 其語法語意都是正確的, crash 只因為上一行被
  //! remark 起來嘛.
  cout << "str= " << *pstr << endl;
  ...
  pA = static_cast<A*>(p);
  //!	pA->A::A(2);				//in VC6 : ctor. this=000307A8
  //! id=2
  // in GCC : [Error] cannot call constructor 'jj02::A::A' directly
...
void test_array_new_and_placement_new() {
...

  {
    ...
    for (int i = 0; i < size; ++i) new (tmp++) A(i);  // 3次 ctor
...

  {
    ...
    A* buf = new A[size];  // default ctor 3 次. [0]先於[1]先於[2])
                           // A必須有 default ctor, 否則 [Error] no matching
                           // function for call to 'jj02::A::A()'
    A* tmp = buf;
...
class Foo {
 ...
 public:
  ...
  //參數是 64, 加了 virtual dtor, sizeof = 16, new Foo[5] => operator new[]() 的
  // size 參數是 84, 上述二例，多出來的 4 可能就是個 size_t 欄位用來放置 array
  // size.
};
...
class Foo {
 public:
  ...
  void* operator new(size_t size, long extra) {
    cout << "operator new(size_t size, long extra)  " << size << ' ' << extra
         << endl;
  ...
  //(5) 這又是一個 placement new, 但故意寫錯第一參數的 type (它必須是 size_t
  //以滿足正常的 operator new)
  //!  	void* operator new(long extra, char init) { //[Error] 'operator new'
...
/*
        ...
        {
                cout << "jjhou global delete(ptr,size), \t" << ptr << "\t" <<
   size << endl; countDel += size; myFree( ptr );
        }
        //(2)
        ...
        inline void  operator delete[](void* ptr, size_t size)
        {
            cout << "jjhou global delete[](ptr,size), \t" << ptr << "\t" << size
   << endl; countArrayDel += size; myFree( ptr );
        }
        //(2)
        inline void  operator delete[](void* ptr)
        {
...
void test_overload_global_new() {
  ...
  //***** 測試時, main() 中的其他測試全都 remark, 獨留本測試 *****
  {
    ...
    string* p = new string(
        "My name is Ace");  // jjhou global new(), 4 	(註：這是
                            // string size) jjhou global new(), 27
                            // (註：這是 sizeof(Rep)+extra)
    ...
    cout << "::countDel= " << ::countDel
         << endl;  // 0 <== 本測試顯然我永遠觀察不到我所要觀察的
                   //       因為進不去 operator delete(ptr,size) 版

    p = new string[3];  // jjhou global new[](), 16 (註：其中內含 arraySize
                        // field: 4 bytes, 所以 16-4 = 12 ==> 4*3, 也就是 3 個
                        // string 每個佔 4 bytes)
    ...
    cout << "::countArrayNew= " << ::countArrayNew
         << endl;  // 16 (這個數值其實對我而言無意義)

    //測試: global operator new 也會帶容器帶來影響
    vector<int> vec(10);  // jjhou global new(), 	40  	0x3e3ea0 (註：10
                          // ints) 註：vector object 本身不是 dynamic allocated.
    vec.push_back(1);
    ...
    lst.push_back(
        1);  // jjhou global new(), 	12	(註：每個 node是 12 bytes)
...

  {
    ...
    cout << "::countNew= " << ::countNew
         << endl;  // 16752832 (注意, node 都不帶 cookie)
...

  {
    ...
    for (int i = 0; i < 1000000; ++i) lst.push_back(i);
    cout << "::countNew= " << ::countNew
         << endl;  // 16000000 (注意, node 都帶 cookie)
...
class __malloc_alloc_template {
 ...
 public:
  ...
  static void (
      *set_malloc_handler(void (*f)()))() {  //類似 C++ 的 set_new_handler().
...
void test_G29_alloc() {
  ...
  //以下, 不能搭配容器來測試, 因為新版 G++ 對於 allocator 有更多要求 (詢問更多
  // typedef 而 alloc 都無法回答) 它其實就是 G4.9 __pool_alloc，所以讓
  // G4.9容器使用 __pool_alloc 也就等同於這裡所要的測試
  /*
...
void test_delete_and_default_for_new() {
  ...
  //! jj12::Foo::operator new [](size_t)'
  //!	delete [] pF;			//[Error] use of deleted function
  //!'static void jj12::Foo::operator delete [](void*, size_t)'

  //! Goo* p2 = new Goo(7);	//[Error] use of deleted function 'static void*
  //! jj12::Goo::operator new(size_t)'
  //!	delete p2;				//[Error] use of deleted
  //! function 'static void jj12::Goo::operator delete(void*, size_t)'
  Goo* pG = new Goo[10];
  delete[] pG;
}
}  // namespace jj12
...
void test_set_new_handler() {
  ...
  /*
      ...
      assert(p);
      p = new int[100000000000000000000];  //[Warning] integer constant is too
...
struct custom_allocator {
  ...
  //     	編譯報錯，new 的語法是 new Type; 不是 new N; 改如下. josuttis
  //     2/e 也如下.
  T* allocate(size_t n) {
    return static_cast<T*>(::operator new(n * sizeof(T)));
  }
  //	void deallocate (T* p, size_t n) { ::delete(p); }
  //		編譯沒報錯，但我認為應使用 operator delete. 改如下. josuttis 2/e
  //也如下.
  void deallocate(T* p, size_t n) { ::operator delete(p); }
};

//以下的 operator== 和 operator!= 原例有,但此處也許因是簡例,即使 mark 起來也
// ok.(目前) josuttis 2/e 中的 custom allocator 也有寫以下二個 operators
template <class T, class U>
constexpr bool operator==(const custom_allocator<T>&,
                          const custom_allocator<U>&) noexcept {
  return true;
}
...
void cookie_test(Alloc&& alloc, size_t n)	//由於呼叫時以 temp obj (Rvalue)
傳入，所以這兒使用 &&. 只是隨意之下的搭配
                                            //使用 &&，那麼呼叫時就不能以 Lvalue
傳入.
*/
...
void cookie_test(Alloc alloc, size_t n) {
  ...
  p1 = alloc.allocate(
      n);  // allocate() and deallocate() 是 non-static, 需以 object 呼叫之.
  p2 = alloc.allocate(n);
  p3 = alloc.allocate(n);
  ...
  alloc.deallocate(
      p2, sizeof(typename Alloc::value_type));  //有些 allocator 對於 2nd
                                                // argument 的值無所謂
...
void test_GNU_allocators() {
  ...
  cout << sizeof(__gnu_cxx::new_allocator<int>)
       << endl;  // 1.
                 //觀察 STL source 可知: new_allocator 是 std::allocator 的 base
                 //我們無法改變 std::allocator 的 base class, 那該如何使用其他的
                 // GNU allocators ? 是否要寫個 custom_allocator (像上面)
                 //並為它加上我想要的 base (例如 __pool_alloc) ?
                 //不，不必，就直接使用, 但需自行 #include <ext/...>

  //從語法上試用各式各樣的 allocators
  cout << sizeof(__gnu_cxx::malloc_allocator<int>)
       << endl;  // 1. 大小 1者其實為 0, fields 都是 static.
  ...
  cout << sizeof(__gnu_cxx::array_allocator<int>)
       << endl;  // 8 ==> 因為它有一個 ptr 指向 array 和一個 size_t 表示消耗到
                 // array 哪兒
  cout << sizeof(__gnu_cxx::debug_allocator<std::allocator<double>>)
       << endl;  // 8
  //!	cout << sizeof(__gnu_cxx::throw_allocator<int>) << endl;	//只有
  //! throw_allocator_base, throw_allocator_random, throw_allocator_limit, 沒有
  //! throw_allocator !!
  ...
  //! vector<int, __pool_alloc<int>> vector_pool;   //如果沒加上 namespace :
  //! [Error] '__pool_alloc' was not declared in this scope
  ...
  cookie_test(__gnu_cxx::malloc_allocator<int>(),
              1);  //相距 10h (表示帶 cookie)
  cookie_test(__gnu_cxx::__pool_alloc<int>(), 1);  //相距 08h (表示不帶 cookie)

  //以下將 int 改為 double 結果不變，意味上述 ints 間隔 8 (而非 4) 乃是因為
  // alignment.
  cookie_test(std::allocator<double>(), 1);  //相距 10h (表示帶 cookie)
  cookie_test(__gnu_cxx::malloc_allocator<double>(),
              1);  //相距 10h (表示帶 cookie)
  cookie_test(__gnu_cxx::__pool_alloc<double>(),
              1);  //相距 08h (表示不帶 cookie)

  cout << "sizeof(array<int,3>) = " << sizeof(array<int, 3>)
       << endl;  // 12 ==> sizeof(_Alloc_block)=16=10h
  cookie_test(__gnu_cxx::bitmap_allocator<array<int, 3>>(),
              1);  //相距 10h (表示不帶 cookie)
                   //!    cookie_test(__gnu_cxx::bitmap_allocator<double>(), 1);
                   //!    //相距 08h (不帶 cookie)

  list<int, __gnu_cxx::__pool_alloc<int>> list_pool;
  for (int i = 0; i < 1000; ++i) list_pool.push_back(i);

  try {
    //示範使用 array_allocator: 須先 new an array, 將其 ptr 設給
    // array_allocator, 最後還要 delete array
    std::tr1::array<double, 100>* arrayTR1 =
        new std::tr1::array<double, 100>;  //使用 tr1::array
    cookie_test(
        __gnu_cxx::array_allocator<double, std::tr1::array<double, 100>>(
            arrayTR1),
        1);  //相距 08h (表示不帶 cookie)
    delete arrayTR1;
    ...
    cookie_test(
        __gnu_cxx::array_allocator<double, array<double, 100>>(arraySTD),
        1);  //相距 08h (表示不帶 cookie)
    delete arraySTD;

    std::tr1::array<double, 1>* p =
        new std::tr1::array<double, 1>;  //為搭配下一行 "default 2nd argument 是
                                         // std::tr1::array<T,1>" (見 source),
                                         //我們需要做一個來.
    cookie_test(
        __gnu_cxx::array_allocator<double>(p),
        1);  //未指明 2nd argument, 所以使用 default, 即 std::tr1::array<T,1>
...
void test_bitmap_allocators() {
  ...
  __gnu_cxx::bitmap_allocator<double>
      alloc;  //形成一個 static __mini_vector<pair>, size=0
